...
import { defaultTeam, useCreateTeam, useTeam, useUpdateTeam } from "@/api/teams";
...
export default function Page(props: PageProps<Params, {}>) {
  ...
  const { data: originTeam } = useTeam(teamId);
  const [team, teamActions] = useTypeSafeReducer(originTeam!, {
  ...
  const updateItem = useUpdateTeam(teamId, diff(team, originTeam!), {
